...
Then(
  ...
  async function (this: World, stepUser: string, stepTable: DataTable): Promise<void> {
    ...
    for (const info of stepTable.hashes()) {
      switch (info.role) {
        case 'manager':
          actualValues = await spacesObject.listMembers({ filter: 'managers' })
          expect(actualValues).toContain(info.user)
          break
        case 'editor':
          actualValues = await spacesObject.listMembers({ filter: 'editors' })
          expect(actualValues).toContain(info.user)
          break
        case 'viewer':
          actualValues = await spacesObject.listMembers({ filter: 'viewers' })
          expect(actualValues).toContain(info.user)
          break
        default:
