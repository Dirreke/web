...
const getThumbnailServiceMock = ({ supportedMimeTypes = [], version = undefined } = {}) => {
...
describe('ThumbnailService', () => {
  describe('availability', () => {
    it('is unavailable if no version given via capabilities', () => {
      expect(getThumbnailServiceMock({ version: undefined }).available).toBe(false)
    })
    it('is available if a version is given via capabilities', () => {
      expect(getThumbnailServiceMock({ version: '1' }).available).toBe(true)
    })
  })
  describe('method "isMimetypeSupported"', () => {
    it('should return true if mimeType is supported', () => {
      const supportedMimeTypes = ['image/png']
      const thumbnailServiceMock = getThumbnailServiceMock({ supportedMimeTypes })
    ...
    it('should return true if no specific supported mimeTypes given', () => {
      const thumbnailServiceMock = getThumbnailServiceMock()
